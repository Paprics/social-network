...
class ChatroomConsumer(WebsocketConsumer):
    def connect(self):
        ...
        # print(f"✅ WebSocket connect: {self.user}")  # <-- вот тут увидишь его в терминале
        # print(f"✅ WebSocket chatroom: {self.chatroom}")  # <-- вот тут увидишь его в терминале
...
class OnlineStatusConsumer(WebsocketConsumer):
    ...
    def online_status_handler(self, event):
        ...
        group_chats_with_users = [
            chat for chat in my_chats.filter(groupchat_name__isnull=False) if chat.users_online.exclude(id=self.user.id)
        ]
